...
export function CustomFilter<TData, TValue>({
  ...
  const getDisplayText = () => {
    ...
    return `${selectedValues.length} ${title.toLowerCase()}${selectedValues.length > 1 ? 's' : ''} selected`;
  };
                ...
                {uniqueValues.map((value) => (
                  ...
                  >
                    <div className="flex items-center space-x-2 w-full">
                      ...
                      <span className="capitalize flex-1 truncate">{value}</span>
                      <span className="ml-auto text-xs text-muted-foreground">
                        {data.filter(item => String(item[fieldKey]) === value).length}
                      </span>
                    </div>
                  </CommandItem>
                ))}
